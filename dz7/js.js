...
const snake = {
  ...
  getNextStepHeadPoint() {
    ...
    switch (this.direction) {
      case 'up':
        return {x: firstPoint.x, y: (firstPoint.y-1) < 0 ? config.getRowsCount() - 1 : (firstPoint.y-1)};
      case 'right':
        return {x: (firstPoint.x+1) >= config.getColsCount() ? 0 : (firstPoint.x+1) , y: firstPoint.y};
      case 'down':
        return {x: firstPoint.x, y: (firstPoint.y+1) >= config.getRowsCount() ? 0 : (firstPoint.y+1) };
      case 'left':
        return {x: (firstPoint.x-1) < 0 ? config.getColsCount() - 1 : (firstPoint.x-1), y: firstPoint.y};
    }
  },
...
const score = {
  ...
  init() {
    ...
    this.countEl = document.querySelector('#score-count');
  },
